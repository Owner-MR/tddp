...
//        SECKILL_ORDER_EXECUTOR.submit(new VoucherOrderHandler());
//    }
//    /** Spring的事务管理注解是基于动态代理实现的，因此，在同一个类中，如果不使用代理对象调用被事务注解管理的方法，不能触发事务。
//     * 解决办法：1. 直接使用AOP，强制在同一个类中使用代理对象来实现被事务管理的方法。
//     * 2. 将该事务方法抽取出来，单独定义一个接口，然后使用接口实例直接调用该方法。
//     * 本质上是注意aop的触发方式--->代理
//     */
...
//            while (true){
//                try {
//                    //获取stream消息队列中的订单信息 XREAD GROUP g1 c1 COUNT 1 BLOCK 2000 STREAM stream.orders > ">代表着最新未消费的消息， $指最新消息"
//                    List<MapRecord<String, Object, Object>> list = stringRedisTemplate.opsForStream().read(
//                            Consumer.from("g1", "c1"),
//                            StreamReadOptions.empty().count(1).block(Duration.ofSeconds(2)),
...
//                    //ack确认消息完成  XACK stream.orders g1 id
...
//            while (true){
//                try {
//                    //获取pendingList消息队列中的订单信息 XREAD GROUP g1 c1 COUNT 1 STREAM stream.orders 0
//                    List<MapRecord<String, Object, Object>> list = stringRedisTemplate.opsForStream().read(
//                            Consumer.from("g1", "c1"),
//                            StreamReadOptions.empty().count(1),
...
//                    VoucherOrder voucherOrder = BeanUtil.fillBeanWithMap(values, new VoucherOrder(), true);
//                    handleVoucherOrder(voucherOrder);
//                    //ack确认消息完成  SACK stream.orders g1 id
//                    stringRedisTemplate.opsForStream().acknowledge(queueName, "g1", record.getId());
//                } catch (Exception e) {
//                    log.error("处理订单异常", e);
//                    try {
